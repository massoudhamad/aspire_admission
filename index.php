...
<head>
    ...
    <!-- Inline registration validation: catches malformed input client-side
         ...
         NECTA to reject it. Server still validates everything. -->
    <style>
        .ichas-field-error  { color:#7A1F16; font-size:13px; font-weight:600; margin-top:4px; display:none; }
        .ichas-field-ok     { border-color:#2D6A4F !important; }
        .ichas-field-bad    { border-color:#C0392B !important; box-shadow:0 0 0 3px rgba(192,57,43,0.18) !important; }
        .ichas-field-error.is-shown { display:block; }
    </style>
    <script type="text/javascript">
        $(document).ready(function () {
            var $form = $('#register');
            ...
            var INDEX_RE = /^[A-Za-z][0-9]{4}\/[0-9]{4}\/[0-9]{4}$/;
            var PHONE_RE = /^(\+?255|0)?7\d{8}$/;
            var EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

            function attachError($input, msg) {
                if ($input.next('.ichas-field-error').length === 0) {
                    $input.after('<div class="ichas-field-error" data-for="' + $input.attr('id') + '"></div>');
                }
                $input.next('.ichas-field-error').text(msg).addClass('is-shown');
                $input.removeClass('ichas-field-ok').addClass('ichas-field-bad');
            }
            function clearError($input) {
                $input.next('.ichas-field-error').removeClass('is-shown').text('');
                $input.removeClass('ichas-field-bad').addClass('ichas-field-ok');
            }
            ...
            $form.on('submit', function (e) {
                ...
                if (!ok) {
                    e.preventDefault();
                    var $first = $form.find('.ichas-field-bad').first();
                    if ($first.length) {
