...
if (!array_key_exists("program_id", $requestData) || !array_key_exists("program_mid", $requestData) || !array_key_exists("admission_id", $requestData)) {
  $this->throwError(INVALID_PARAMETERS_REQUEST, INVALID_PARAMETERS_REQUEST_MESSAGE);
}

// Check Parameters
if (empty($requestData['program_id']) || empty($requestData['program_mid']) || empty($requestData['admission_id'])) {
  $this->throwError(REQUIRED_USER_PASS, REQUIRED_USER_PASS_MESSAGE);
}

$program_id = $requestData['program_id'];
$program_mid = $requestData['program_mid'];
$admission_id = $requestData['admission_id'];


// Get All Addission Programmes Major
if ($program_mid == 'all') {
  $applicants = $db->getAllBatchRegistered($program_id, $admission_id);
} else {
  $applicants = $db->getRegisteredBatchApplicantsAllDetails($program_mid, $admission_id);
}
...
// echo '{';
// echo '"progMajorName":' . json_encode($programmemajorName) . ',';
// echo '"admissionName":' . json_encode($admissionName) . ',';
// echo '"registered":[';
...
// $x = '{';
// $x .= '"progMajorName":' . json_encode($programmemajorName) . ',';
// $x .= '"admissionName":' . json_encode($admissionName) . ',';
// $x .= '"registered":[';
// foreach ($allApplicants as $data) {
//   $x .= json_encode($data) . ',';
// }
